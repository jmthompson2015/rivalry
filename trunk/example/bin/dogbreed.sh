...
OUTPUT_FILE=dogbreed.out

java -cp ${CP} org.rivalry.example.dogbreed.BreedNameDataCollectorMain &> dogbreed0.out
java -cp ${CP} org.rivalry.example.dogbreed.DogBreedDataCollectorMain &> ${OUTPUT_FILE}
...
if [ ${LINE_COUNT} -eq 594 ];
then
	cp -f DogBreedRivalryData.xml ${USER_DIR}/Dropbox/Public/rivalry 
fi
