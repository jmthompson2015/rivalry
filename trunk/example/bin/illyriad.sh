...
java -cp ${CP} org.rivalry.example.illyriad.IllyriadDataCollectorMain -u "Zarek Lock" -p tristugo -U Frak -P tristugo &> illyriad.out

echo "line count: " `cat illyriad.out | wc -l`

LINE_COUNT=`cat illyriad.out | wc -l`
echo "LINE_COUNT = " ${LINE_COUNT}

if [ ${LINE_COUNT} -eq 83 ];
then
	cp -f IllyriadRivalryData.xml ${USER_DIR}/Dropbox/Public/rivalry 
fi
