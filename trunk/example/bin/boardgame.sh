...
OUTPUT_FILE=boardgame.out

java -cp ${CP} org.rivalry.example.boardgame.BoardgameNameDataCollectorMain &> boardgame0.out
java -cp ${CP} org.rivalry.example.boardgame.BoardgameDataCollectorMain &> ${OUTPUT_FILE}
...
if [ ${LINE_COUNT} -eq 308 ];
then
	cp -f BoardgameRivalryData.xml ${USER_DIR}/Dropbox/Public/rivalry 
fi
