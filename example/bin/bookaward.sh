...
OUTPUT_FILE=bookaward.out

java -cp ${CP} org.rivalry.example.bookaward.MysteryAwardDataCollectorMain &> ${OUTPUT_FILE}
...
if [ ${LINE_COUNT} -eq 79 ];
then
	cp -f MysteryAwardRivalryData.xml ${USER_DIR}/Dropbox/Public/rivalry 
fi
