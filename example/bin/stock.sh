...
java -cp ${CP} org.rivalry.example.stock.StockDataCollectorMain &> stock.out

echo "line count: " `cat stock.out | wc -l`

LINE_COUNT=`cat stock.out | wc -l`
echo "LINE_COUNT = " ${LINE_COUNT}

if [ ${LINE_COUNT} -eq 52 ];
then
	cp -f StockRivalryData.xml ${USER_DIR}/Dropbox/Public/rivalry 
fi
